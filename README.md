...
- **Flow states**: Ordered states (e.g. Backlog, In Progress, Done) classified as `Active`, `Completed`, or `Cancelled`. Only `Active` states carry an order and render as board columns; `Completed` and `Cancelled` are terminal.
...
- **Read-only afterwards**: there are no endpoints to edit a project's flow. The `Active` flow states are surfaced through the board (`GET /api/v1/flowboard/projects/{projectId}/board`); the terminal ones only through a work item's available transitions.

### Work Items

- **Create work item** (`POST /api/v1/flowboard/workitems`): Open a new work item with title, description, type, and priority.
- **Board view** (`GET /api/v1/flowboard/projects/{projectId}/board`): In-flight work items for a project grouped by the project's `Active` flow states (Kanban board shape), ordered by state and priority. Items in a `Completed` or `Cancelled` state are not on the board.
