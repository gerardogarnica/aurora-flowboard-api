...
| **FlowState**         | A single state within a flow (e.g. Backlog, In Progress, Code Review, QA, Done). Every state declares a `FlowStateCategory`.                                                                                                                                                                             | Configuration            |     |
| **FlowStateCategory** | Classification of a flow state: `Active`, `Completed`, `Cancelled`. `Active` states are the in-flight, board-visible ones and are the only ones that carry an explicit order. `Completed` and `Cancelled` are the terminal states — work that reaches them is finished or dropped, and leaves the board. | Configuration            |     |
| **Board**             | A Kanban visualization of the in-flight work items in a project, grouped by its `Active` flow states.                                                                                                                                                                                                    | View                     |     |
...
- Every state declares a `FlowStateCategory` — `Active`, `Completed` or `Cancelled`. "Terminal", used throughout this document, means a state in the `Completed` or `Cancelled` category. A flow must contain at least one state of each category.
- Ordering is a property of the `Active` states only: they carry a `SortOrder` assigned in creation order and that order is the board's column order. Terminal states are unordered — they are destinations, not positions in the pipeline.
...
| Board               | Scope                                                     | Purpose                     |
| ------------------- | --------------------------------------------------------- | --------------------------- |
| **Project board**   | Work items in a project sitting in an `Active` flow state | Day-to-day operational view |
...
- Columns are the `Active` flow states
...
- Board columns are the project's `Active`-category flow states, in `SortOrder` order. Because a project has a single flow, every board within it has the same columns — there is no union of states to reconcile and no ambiguity in column ordering.
- Terminal states (`Completed`, `Cancelled`) never render as columns. Moving a card to a terminal state removes it from the board; the board is a view of in-flight work, not an archive. Closed items stay reachable by code (`GET /work-items/{code}`) and are counted in the project summaries (§5.1), but there is no board column that accumulates them.
...
- The board shows only `Active` states (§5.8), so closed work is reachable only by code. Does that leave a gap — a "recently closed" list, a done-this-sprint view, or a filterable work item search — or is the project summary count enough?
