...
public sealed class ActivateMilestone : IBaseEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPatch(
            "milestones/{id:guid}/activate",
            async (
                Guid id,
                ICommandHandler<ChangeMilestoneStatusCommand> handler,
                CancellationToken cancellationToken) =>
            {
                var command = new ChangeMilestoneStatusCommand(id, MilestoneStatus.Active);
