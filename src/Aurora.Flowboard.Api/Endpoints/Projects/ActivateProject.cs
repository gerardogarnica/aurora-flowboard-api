...
public sealed class ActivateProject : IBaseEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPatch(
            "projects/{id:guid}/activate",
            async (
                Guid id,
                ICommandHandler<ChangeProjectStatusCommand> handler,
                CancellationToken cancellationToken) =>
            {
                var command = new ChangeProjectStatusCommand(id, ProjectStatus.Active);
