...
public sealed class RemoveFlowStateHandlerTests
{
    ...
    public async Task Should_ReturnSuccess_When_CommandIsValid()
    {
        ...
        Flow flow = FlowCommandData.GetFlowWithStates(admin);
        FlowState activeState = flow.States.First(s => s.Category == FlowStateCategory.Active);
        ...
        RemoveFlowStateCommand command = new(flow.Id, activeState.Id);
    ...
    public async Task Should_ReturnDomainError_When_StateDoesNotExist()
    {
        ...
        RemoveFlowStateCommand command = new(flow.Id, Guid.NewGuid());
